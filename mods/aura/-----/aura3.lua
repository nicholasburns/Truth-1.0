...
TruthAuras:SetSize(40, 40)
TruthAuras:Template('DEFAULT')
-- TruthAuras:RegisterEvent('PLAYER_ENTERING_WORLD')	-- TruthAuras:RegisterEvent('UNIT_AURA')	-- T.MakeMovable(TruthAuras, 'HIGH')
-- TruAura:SetScript('OnEvent', OnEvent)

local function OnLoad(self)
	self:Template('DEFAULT')					-- self:SetBackdropBorderColor(ACHIEVEMENTUI_GOLDBORDER_R, ACHIEVEMENTUI_GOLDBORDER_G, ACHIEVEMENTUI_GOLDBORDER_B, ACHIEVEMENTUI_GOLDBORDER_A)
	self.auras = {}
	...
	if (event == "UNIT_AURA") then
		...
		for i = 1, 40 do
			...
			if (name == Anticipation) then
				...
				exp, dur = expiration, duration
			  -- aura.start, aura.duration = expiration, duration

				if (aura.duration > 0) then			-- obj:SetCooldown(start, duration [charges, maxCharges])
				-- if (dur > 0) then			-- obj:SetCooldown(start, duration [charges, maxCharges])
					CooldownFrame_SetTimer(aura.cooldown, exp - dur - 0.5, dur - 0.5)
					aura.cooldown:Show()

					-- CooldownFrame_SetTimer(aura.cooldown, aura.start, aura.duration)
					-- aura.cooldown:SetCooldown(exp - dur - 0.5, dur - 0.5)
